//
//  main.cpp
//*********************************************************************
// Program name: Final project
// Author: YUCEN CAO - CS162 Spring 2019
// Date: June 11, 2019
// Description: In the final project, you will be able to design and implement a one-player, text-based game where the player can move through spaces to get items and accomplish goals.
// It is a collecting item game, when the player collect all the item, it will win. 
// The max step is 11, step will reduce by each move.
// Every room has different items, and some of the room is very dangerous
// *********************************************************************

#include <iostream>
#include <vector>
#include "game.hpp"
using std::vector;

int main() {
	vector<char> container; 
	int step = 11;
	int win = -1;
	
	win = game(&container, step);
	if (win == 1)
	{
		std::cout << std::endl;
		std::cout << "**********************************************" << std::endl;
		std::cout << "-----------You Collect all the item ----------" << std::endl;
		std::cout << "-------------WOW, You win the game------------" << std::endl;
		std::cout << "**********************************************" << std::endl;
	}
	else{
		std::cout << std::endl;
		std::cout << "**********************************************" << std::endl;
		std::cout << "--------------There is no step left ----------" << std::endl;
		std::cout << "-------------Sadly, You lose the game---------" << std::endl;
		std::cout << "**********************************************" << std::endl;
	}
	
	
	return 0;
}
